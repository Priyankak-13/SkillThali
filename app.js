...
// ============ NAVBAR VISIBILITY ============
function updateNavbarVisibility(activeId) {
  const navbarVisibleSections = ['home'];
  const mainNav = document.getElementById('mainNav');
  if (!mainNav) return;
  if (navbarVisibleSections.includes(activeId)) {
...
function goToDashboard(type) {
  // Capture name from signup form if available
  ...
  if (!currentUser.name) {
    ...
    if (emailEl && emailEl.value.trim()) {
      const raw = emailEl.value.split('@')[0];
  ...
  if (type === 'student') {
    ...
    updateWelcomeMessages('student');
  } else {
    document.getElementById('clientDash').classList.add('active');
    updateNavbarVisibility('clientDash');
    renderClientTasks();
    updateWelcomeMessages('client');
  }
  window.scrollTo(0, 0);
}

// ============ DYNAMIC WELCOME MESSAGE ============
function updateWelcomeMessages(role) {
  const name = currentUser.name || (role === 'student' ? 'Sakshi' : 'Rahul');
  const isNew = currentUser.isNew;
  const greeting = isNew ? `Welcome, ${name} 🎉` : `Welcome back, ${name} 👋`;

  if (role === 'student') {
    const el = document.getElementById('studentUsername');
    if (el) el.textContent = name;
    const greetEl = document.getElementById('studentGreeting');
    if (greetEl) greetEl.textContent = greeting;
    // Update profile dropdown name
    const dropName = document.getElementById('profileDropName');
    if (dropName) dropName.textContent = name;
    const dropInitials = document.getElementById('studentAvatarInitials');
    if (dropInitials) dropInitials.textContent = name.substring(0, 2).toUpperCase();
    const navInitials = document.getElementById('studentNavInitials');
    if (navInitials) navInitials.textContent = name.substring(0, 2).toUpperCase();
  } else {
    const el = document.getElementById('clientUsername');
    if (el) el.textContent = name;
    const greetEl = document.getElementById('clientGreeting');
    if (greetEl) greetEl.textContent = greeting;
    const dropName = document.getElementById('clientProfileDropName');
    if (dropName) dropName.textContent = name;
    const dropInitials = document.getElementById('clientAvatarInitials');
    if (dropInitials) dropInitials.textContent = name.substring(0, 2).toUpperCase();
    const navInitials = document.getElementById('clientNavInitials');
    if (navInitials) navInitials.textContent = name.substring(0, 2).toUpperCase();
  }
}

function logout() {
  currentUser = { name: '', role: '', isNew: false };
  // Close any open dropdowns
  const drops = document.querySelectorAll('.profile-dropdown-menu');
...
function toggleStudentProfileDrop(e) {
  ...
  const clientMenu = document.getElementById('clientProfileMenu');
  if (clientMenu) clientMenu.classList.remove('open');
}

function toggleClientProfileDrop(e) {
  e.stopPropagation();
  const menu = document.getElementById('clientProfileMenu');
  if (menu) menu.classList.toggle('open');
  const studentMenu = document.getElementById('studentProfileMenu');
  if (studentMenu) studentMenu.classList.remove('open');
}
...
function submitTask() {
  ...
  const newTask = {
    ...
    desc, client: currentUser.name || 'Rahul Mehta', location: 'Pune, MH',
...
function showToast(msg) {
  ...
  t.style.cssText = `
    ...
    background:linear-gradient(135deg,rgba(22,26,48,0.97),rgba(49,48,77,0.96));
    border:1px solid rgba(182,187,196,0.2);border-radius:50px;
    padding:13px 28px;font-size:0.9rem;color:#F0ECE5;
