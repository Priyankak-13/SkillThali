...
function saveStudentActivity(a) { try { localStorage.setItem('st_activity_' + (currentUser.email || 'anon'), JSON.stringify(a)); } catch(e) {} }
function loadStudentActivity() { try { const d = localStorage.getItem('st_activity_' + (currentUser.email || 'anon')); return d ? JSON.parse(d) : { applied: [], active: [], completed: [], submitted: [] }; } catch(e) { return { applied: [], active: [], completed: [], submitted: [] }; } }
  ...
  tasks.forEach((task, i) => {
    ...
    card.innerHTML = `
      ...
      <div class="task-meta">
        ...
        ${paySecured && !applied ? '<span class="payment-badge secured">🔒 Payment Ready</span>' : ''}
      </div>`;
...
function approveAndRelease(taskId) {
  ...
  // Update student activity if we can find them
  // In a real app we'd look up by assignedTo email; here we just update global
  saveTaskStore();
