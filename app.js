/* ================================================
   SkillThali — app.js (Full Enhanced Version v3)
   FIXES APPLIED:
   - Delete task: removes from client AND all student dashboards
   - Pay Now: only after work submission; on click → Payment Secured → moves to Completed
   - New theme colors: #441752 → #8174A0, buttons #EFB6C8, hover #A888B5
================================================ */
...
// ============ CLIENT SUBMITTED TASKS — FIX: Pay Now only after submission, on click → completed ============
function renderClientSubmittedTasks() {
  const container = document.getElementById('clientSubmittedTasksList');
  if (!container) return;
  container.innerHTML = '';
  // Only show tasks with status === 'submitted' (student has uploaded work)
  ...
  submitted.forEach(task => {
    ...
    // FIX: Show "Pay Now" only when status=submitted AND paymentStatus=pending
    // When already secured, show "Release Payment" button
    const payBlock = task.budget > 0 ? (
      task.paymentStatus === 'pending'
        ...
        : task.paymentStatus === 'secured'
          ? `<button class="btn-approve-release" onclick="event.stopPropagation();approveAndRelease(${task.id})">✅ Release Payment to Student</button>`
          : ''
    ) : `<button class="btn-approve-release" onclick="event.stopPropagation();approveCollab(${task.id})">✅ Mark as Complete</button>`;
...
// ============ SIMULATE PAYMENT — FIX: on confirm → immediately move to Completed ============
function simulatePayment(taskId) {
  ...
  if (overlay) {
    ...
    if (body) {
      body.innerHTML = `
        <button class="btn-primary full" id="paySimConfirmBtn" onclick="confirmPayment(${taskId})">🔒 Pay ₹${total.toLocaleString('en-IN')} & Approve</button>
...
// FIX: confirmPayment now secures payment, shows success, then immediately releases and moves to Completed
function confirmPayment(taskId) {
  ...
  setTimeout(() => {
    // Mark as secured then immediately release (pay → approve in one step)
    task.paymentStatus = 'secured';
    saveTaskStore();

    setTimeout(() => {
      // Auto-release: move to completed
      task.status = 'completed';
      task.paymentStatus = 'released';
      _moveStudentToCompleted(taskId, task);
      saveTaskStore();
      ...
      if (body) {
        body.innerHTML = `
          <div class="pay-success">
            ...
            <h3>Payment Secured & Approved!</h3>
            ...
            <p style="margin-top:12px;font-size:0.85rem;color:var(--green)">✅ Task moved to Completed. Student will be notified.</p>
            <button class="btn-primary" onclick="closePaymentModal()" style="margin-top:16px;width:100%">Done</button>
          </div>`;
      }

      addNotification(currentUser.email, `✅ Payment of ₹${total.toLocaleString('en-IN')} sent for "${task.title}". Task completed!`, 'success');
      renderClientSubmittedTasks();
      renderClientCompletedTasks();
      updateDynamicUI('client');
      showToast(`💰 Payment approved for "${task.title}"! Task moved to Completed.`);
    }, 800);
  }, 1500);
}
...
// ============ APPROVE & RELEASE PAYMENT (manual, from modal) ============
function approveAndRelease(taskId) {
  ...
  renderClientSubmittedTasks();
  renderClientCompletedTasks();
  showToast(`💰 Payment released for "${task.title}"! ₹${task.budget.toLocaleString('en-IN')} sent to ${task.assignedTo || 'student'}.`);
}

// ============ DELETE TASK — FIX: removes from client AND all student dashboards ============
function deleteTask(e, id) {
  ...
  // Remove from global task store
  taskStore = taskStore.filter(t => t.id !== id);

  // Remove from ALL registered users' activity storage (persisted)
  const allUsers = loadUsers();
  allUsers.forEach(u => {
    const key = getActivityKey(u.email);
    try {
      ...
      if (data) {
        ...
        if (act.active && act.active.length >= 0) {
          addNotification(u.email, `⚠️ Task "${taskTitle}" was removed by the client.`, 'warning');
        }
      }
    } catch(e2) {}
  });

  // Clean current user's in-memory activity
  ...
  saveTaskStore();
  renderClientTasks();
  // Force refresh available tasks for current student session if applicable
  renderAvailableTasks('all');
  showToast(`🗑️ "${taskTitle}" deleted and removed from all dashboards.`);
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (saved && saved.name) {
    currentUser = saved;
    studentActivity = loadStudentActivity();
    if (currentUser.role === 'admin') { goToAdminDash(); return; }
  }
